...
public class SearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //Get list book, category and author
        ...
        System.out.println("---------------------Search request from user: "+searchRequest+"------------------------");
        List<Book> bookFoundByRequest = new ArrayList<Book>();

        for(Book book : allBook) {
            if(BookDB.getInstance().checkSearchRequestRelatedWithBook(book, searchRequest)) {
                bookFoundByRequest.add(book);
            }
        }
        request.setAttribute("books", bookFoundByRequest);
        request.setAttribute("nameOfCategory","Trang tìm kiếm");
